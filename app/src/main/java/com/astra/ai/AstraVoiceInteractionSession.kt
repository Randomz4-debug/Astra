...
class AstraVoiceInteractionSession(context: android.content.Context) : VoiceInteractionSession(context) {
    ...
    override fun onCreateContentView(): View {
        ...
        send = Button(context).apply { text = "Send"; setOnClickListener { submit(input.text.toString()) } }
        row.addView(mic, LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f))
        ...
        recognizer = SpeechRecognizer.createSpeechRecognizer(context).also { r ->
            r.setRecognitionListener(object : RecognitionListener {
                ...
                override fun onResults(results: Bundle?) {
                    ...
                    if (text.isNotBlank()) { input.setText(text); submit(text) }
                }
    ...
    private fun submit(text: String) {
        ...
        Thread {
            ...
            post { status.text = "Astra"; tts?.speak(answer, TextToSpeech.QUEUE_FLUSH, null, "astra-assistant"); input.setText(answer) }
        }.start()
    }
