...
/**
 * Local/LAN AI gateway. Handles native Ollama, OpenAI-compatible servers and
 * the common mistake of using Ollama's API on port 12434 instead of 11434.
 */
class LocalAiGateway(private val context: Context) {
    ...
    fun configure(endpoint: String, model: String) {
        prefs.edit()
            .putString("endpoint", normalize(endpoint))
            .putString("model", model.trim())
            .apply()
    }

    /** Returns every model found on the configured server. */
    suspend fun discoverOllamaModels(): List<String> = withContext(Dispatchers.IO) {
        ...
        for (base in candidates) {
            ...
            val arr = runCatching { JSONObject(text).optJSONArray("models") }.getOrNull() ?: continue
            for (i in 0 until arr.length()) {
                arr.optJSONObject(i)?.optString("name")?.takeIf { it.isNotBlank() }?.let(result::add)
            }
    ...
    /** Discover both native Ollama models and OpenAI-compatible /v1/models models. */
    suspend fun discoverModels(): List<String> = withContext(Dispatchers.IO) {
        ...
        for (base in candidates) {
            ...
            val ollama = request("$base/api/tags", "GET", null)
            parseModelIds(ollama.orEmpty()).forEach(result::add)

            val compatible = request("$base/v1/models", "GET", null)
            parseModelIds(compatible.orEmpty()).forEach(result::add)
        }
        result.toList()
    }

    /** Returns a human-readable diagnosis rather than silently reporting no models. */
    suspend fun diagnose(): String = withContext(Dispatchers.IO) {
        val configured = normalize(endpoint())
        val candidates = endpointCandidates(configured)
        for (base in candidates) {
            val host = runCatching { URL(base).host }.getOrNull() ?: continue
            if (!isAllowedHost(host)) continue
            val tags = request("$base/api/tags", "GET", null)
            if (tags != null) {
                val count = parseModelIds(tags).size
                return@withContext if (count > 0) {
                    "Connected to Ollama at $base • $count model(s) found."
                } else {
                    "Connected to Ollama at $base, but no models are installed. Run `ollama list` / `ollama pull <model>`."
                }
            }
            val compatible = request("$base/v1/models", "GET", null)
            if (compatible != null) {
                val count = parseModelIds(compatible).size
                return@withContext if (count > 0) {
                    "Connected to OpenAI-compatible server at $base • $count model(s) found."
                } else "Server is reachable at $base, but it returned no models."
            }
        }
        if (configured.contains(":12434")) {
            return@withContext "Could not reach $configured. Astra also tried the standard Ollama port 11434. If Ollama is running on your PC, use http://YOUR-PC-IP:11434 and allow Ollama through Windows Firewall."
        }
    ...
    suspend fun chat(prompt: String): String = withContext(Dispatchers.IO) {
        ...
        for (base in candidates) {
            val uri = runCatching { URL(base) }.getOrElse { continue }
            if (!isAllowedHost(uri.host)) {
                lastError = "Local-only mode blocked a non-local AI endpoint."
                continue
            }
            val models = discoverModelsFromBase(base)
            val chosen = model().ifBlank { models.firstOrNull().orEmpty() }
            if (chosen.isBlank()) {
                lastError = "No local model found. Install a model in Ollama, then tap Discover Models."
                continue
            }
            ...
            if (response == null) {
                lastError = "Local AI at $base could not answer."
                continue
            }
            val json = runCatching { JSONObject(response) }.getOrElse {
                lastError = "Local AI returned invalid JSON."
                continue
            }
            if (json.optString("error").isNotBlank()) {
                lastError = "Local AI error: ${json.optString("error")}"
                continue
            }
    ...
    private fun endpointCandidates(value: String): List<String> {
        ...
        // 12434 is a common typo/configuration left over from unrelated local services.
        // If the user entered it, transparently try Ollama's standard API port too.
        runCatching {
            val u = URL(clean)
    ...
    private fun isAllowedHost(host: String): Boolean {
        ...
        return runCatching {
            InetAddress.getByName(normalized).let { it.isSiteLocalAddress || it.isLinkLocalAddress }
        }.getOrDefault(false)
    }
}
