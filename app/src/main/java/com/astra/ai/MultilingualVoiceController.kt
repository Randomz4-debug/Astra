...
class MultilingualVoiceController(private val context: Context) : TextToSpeech.OnInitListener {
    ...
    init { if (SpeechRecognizer.isRecognitionAvailable(context)) recognizer = SpeechRecognizer.createSpeechRecognizer(context) }
    ...
    fun listen(language: String, onResult: (String) -> Unit, onState: (Boolean) -> Unit) {
        ...
        startRecognition(language, onResult, onState)
    }

    fun startAlwaysListening(language: String = "auto", onResult: (String) -> Unit) {
        continuous = true
        continuousCallback = onResult
        startRecognition(language, onResult) { VoiceTelemetry.setListening(it) }
    }
    ...
    fun stop() { continuous = false; continuousCallback = null; handler.removeCallbacksAndMessages(null); runCatching { recognizer?.cancel() }; VoiceTelemetry.setListening(false); VoiceTelemetry.setRms(0f) }
    ...
    fun setVoice(voiceName: String): Boolean = runCatching { val selected = tts?.voices?.firstOrNull { it.name == voiceName } ?: return false; tts?.voice = selected; true }.getOrDefault(false)
    fun availableVoices(): List<Voice> = tts?.voices?.toList().orEmpty()
    fun release() { stop(); recognizer?.destroy(); recognizer = null; tts?.stop(); tts?.shutdown(); tts = null }
}
