...
/**
 * Central voice controller for Astra. Recognition and TTS are kept on Android's
 * main thread as required by the platform, and TTS requests made before engine
 * initialization are queued instead of being silently dropped.
 */
class MultilingualVoiceController(private val context: Context) : TextToSpeech.OnInitListener {
    private var tts: TextToSpeech? = TextToSpeech(context, this)
    private var ttsReady = false
    private var pendingSpeech: Pair<String, String>?
    ...
    private fun startRecognition(language: String, onResult: (String) -> Unit, onState: (Boolean) -> Unit) {
        ...
        sr.setRecognitionListener(object : RecognitionListener {
            ...
            override fun onResults(b: Bundle?) {
                onState(false)
                VoiceTelemetry.setListening(false)
    ...
    private fun restart() {
        if (continuous) handler.postDelayed({
            if (continuous) startRecognition(languageTag, continuousCallback ?: {}, { VoiceTelemetry.setListening(it) })
        }, 350)
    }
    ...
    fun speak(text: String, language: String = "auto") {
        ...
        if (!ttsReady) {
            pendingSpeech = clean to language
            return
        }
