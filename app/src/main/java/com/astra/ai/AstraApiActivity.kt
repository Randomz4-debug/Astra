...
private fun ApiManagerScreen(hub: AstraApiHub) {
    ...
    fun save() {
        ...
        runCatching {
            ...
            apis = hub.all(); status = "Saved $name. Astra can now use this API."
        }.onFailure { status = "Could not save: ${it.message}" }
    }

    LazyColumn(Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        item {
            Text("ASTRA API HUB", style = MaterialTheme.typography.headlineMedium)
            Text("Add as many REST APIs as you want. Astra can use them by name from voice or chat.", color = Color.LightGray)
