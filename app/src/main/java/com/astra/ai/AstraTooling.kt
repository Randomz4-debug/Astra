...
class DeviceTools(private val context: Context) {
    ...
    fun camera(): ToolResult = runCatching { context.startActivity(Intent(android.provider.MediaStore.ACTION_IMAGE_CAPTURE).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)); ToolResult(true, "Opening the camera.") }.getOrElse { ToolResult(false, "No camera app is available.") }
...
class LocalCommandEngine(private val context: Context) {
    ...
    suspend fun handle(text: String): ToolResult? {
        val t = text.trim(); val lower = t.lowercase()
        return when {
