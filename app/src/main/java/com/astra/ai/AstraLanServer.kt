...
class AstraLanServer(private val context: Context) {
    ...
    fun start(port: Int = 8765): Boolean {
        ...
        return try {
            ...
            job = scope.launch { while (true) { val client = socket?.accept() ?: break; launch { handle(client) } } }
            true
    ...
    private suspend fun route(method: String, rawPath: String, body: String): Pair<Int, String> {
        ...
        return when {
            ...
            method == "POST" && path == "/api/chat" -> runCatching {
                val o = JSONObject(body); val answer = runtime.handle(o.optString("prompt"), o.optBoolean("localOnly", false))
    ...
    private suspend fun parallel(body: String): Pair<Int, String> = runCatching {
        ...
        val parsed = buildList { for (i in 0 until tasks.length()) add(tasks.optString(i)) }.filter { it.isNotBlank() }
        if (parsed.isEmpty()) return@runCatching 400 to JSONObject().put("error", "tasks is empty").toString()
        200 to JSONObject().put("results", JSONArray(ParallelAgentOrchestrator(appContext).run(parsed))).toString()
    }.getOrElse { 400 to JSONObject().put("error", it.message ?: "bad request").toString() }

    private suspend fun providerChat(body: String): Pair<Int, String> = runCatching {
        val o = JSONObject(body); val result = registry.chat(o.getString("providerId"), o.getString("prompt"))
...
<body><h1>ASTRA LAN</h1><p>Wi-Fi/LAN control panel. Devices on the same network can use this UI and JSON API.</p>
