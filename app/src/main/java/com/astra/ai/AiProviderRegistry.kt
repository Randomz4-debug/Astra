...
class AiProviderRegistry(context: Context) {
    ...
    suspend fun discoverJson(): String = withContext(Dispatchers.IO) {
        ...
        for (provider in providers()) {
            ...
            var found = false
            for (url in urls) {
                val result = request(url, key, null, "GET") ?: continue
                val models = parseModels(result)
                if (models.length() > 0) {
                    for (i in 0 until models.length()) {
                        val m = models.getJSONObject(i); m.put("providerId", provider.optString("id")); m.put("providerName", provider.optString("name")); all.put(m)
                    }
                    found = true; break
                }
            }
    ...
    suspend fun chat(id: String, prompt: String): String = withContext(Dispatchers.IO) {
        ...
        val model = bestModel(models)
        require(model.isNotBlank()) { "No model available" }
    ...
    private fun bestModel(models: JSONArray): String {
        var best = ""; var bestScore = Int.MIN_VALUE
        for (i in 0 until models.length()) {
            val id = models.optJSONObject(i)?.optString("id") ?: continue
            val n = id.lowercase(); var score = 0
    ...
    private fun parseModels(text: String): JSONArray {
        val o = runCatching { JSONObject(text) }.getOrNull() ?: return JSONArray(); val source = o.optJSONArray("models") ?: o.optJSONArray("data") ?: return JSONArray(); val out = JSONArray()
    ...
    private fun parseChat(text: String): String = JSONObject(text).optJSONObject("message")?.optString("content")?.takeIf { it.isNotBlank() }
        ?: JSONObject(text).optJSONArray("choices")?.optJSONObject(0)?.optJSONObject("message")?.optString("content")?.takeIf { it.isNotBlank() }
        ?: JSONObject(text).optString("response").takeIf { it.isNotBlank() } ?: "No response returned."

    private fun request(url: String, key: String?, body: String?, method: String): String? = runCatching {
        val c = URL(url).openConnection() as HttpURLConnection; c.requestMethod = method; c.connectTimeout = 5000; c.readTimeout = 30000; c.setRequestProperty("Accept", "application/json")
        ...
        val stream = if (c.responseCode in 200..299) c.inputStream else c.errorStream; stream?.bufferedReader()?.use { it.readText() }.also { c.disconnect() }
    }.getOrNull()
}
