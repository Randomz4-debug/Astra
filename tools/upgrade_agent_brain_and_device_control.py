...
replacement2 = '''        val chatId = currentChatId()
        ...
        // Execute compound requests before the simple command router.'''
if 'shouldUseAgenticExecutor' not in s:
    if needle2 not in s:
...
'''
if 'private fun shouldUseAgenticExecutor' not in s:
...
if 'private fun findNormalized' not in s:
    s = s.replace(needle2, helper + needle2)
p.write_text(s, encoding='utf-8')
...
# Support "open WhatsApp, open +number, read..." as one deterministic task.
if 'open\\s+(.+?)\\s*[,;]\\s*(?:then\\s+)?open' not in s:
    old = '''        val readChat = Regex(
    ...
    # The exact escaped source is easier to replace using a smaller marker.
...
'''
    if marker in s: s = s.replace(marker, extra)
# Remove the wa.me side effect from prepareOpenedApp.
start = s.find('    private fun prepareOpenedApp(')
if start >= 0:
    end = s.find('    private fun buildReasoningRequest', start)
    if end > start:
        s = s[:start] + '    private fun prepareOpenedApp(step: String, wholeRequest: String) { /* WhatsApp navigation stays inside the app UI. */ }\n\n' + s[end:]
p.write_text(s, encoding='utf-8')
